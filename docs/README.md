...
`ResultService` использует единый пайплайн анализа результатов:

- данные из CSV читаются через общий перечислитель строк и парсятся в `TestResult`;
- агрегаты (`AnalyzedResults`) для одиночного отчета и summary-отчетов собираются одинаково;
- общий flow summary-отчетов используется и для набора CSV-файлов, и для словаря `TestResult` в памяти;
- графики по времени для CSV строятся отдельным проходом после определения полного временного диапазона, чтобы интервалы считались от реального минимального `Timestamp`.
